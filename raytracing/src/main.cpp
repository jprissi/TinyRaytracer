...
bool propagate_ray(Vect *p_outgoing_ray_origin, Vect *p_outgoing_ray_dir, Vect *p_color)
{
  ...
  for (const auto &object : scene.scene_objects)
  {
    if (object->is_hit(ray_origin, ray_direction, *p_outgoing_ray_origin, *p_outgoing_ray_dir, distance_to_hit, hit_material, hit_color, ray_energy, scene.lightSource))
    {
      object_hit = true;
...
int main()
{
  ...
  for (int y = 256; y >= -255; y--)
  {
    for (int x = -255; x <= 256; x++)
    {
      ...
      for (int i = 0; i < AA_samples; i++)
      {
        ...
        ray_energy = 1.0f;//0.2*random_offset()+0.8f;//1.0f;//random_offset(); //1.0f;

        //adding depth of field

        Vect sensor_shift{random_offset() * camera.aperture, random_offset() * camera.aperture, 0.0f};
        ray_origin = ray_origin + sensor_shift;
        ray_direction = !(ray_direction - sensor_shift * (1. / camera.focalDistance));
  ...
  unsigned error = lodepng::encode(filename, img, camera.getWidth(), camera.getHeight());
